...
class DistilbertSentimentPreprocesser():
  ...
  def __call__(self, context:str) -> Tuple[torch.Tensor, torch.Tensor, int]:

    #tokenize inputs (question + context, with special tokens)
    context_ids = self.tokenizer.encode(context, add_special_tokens=False)
    input_ids_raw = [self.cls_token_id] + context_ids + [self.sep_token_id]

    #tokenize baseline (independent of input - necessary for integrated gradients)
    baseline_input_ids_raw = [self.cls_token_id] + [self.baseline_token_id] * len(context_ids) + [self.sep_token_id]
