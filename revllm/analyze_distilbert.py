...
def get_top_k_attributed_tokens(attrs: torch.Tensor, k:int, preprocessor) -> Tuple[List[str], torch.Tensor, torch.Tensor]:
...
class AnalyzeQAndA():
    ...
    def predict(self, input_ids=None, attention_mask = None, position:int = 0) -> None:
        ...
        #for use in lig.attribute below, which need these args to be passed in
            ...
            pred = pred[position]
            #todo: edit this for readability
    ...
    def lig_top_k_tokens(self, k:int=5) -> None:
        ...
        attributions_start = lig.attribute(inputs=(self.preprocessor.input_ids),
                                          baselines=(self.preprocessor.baseline_input_ids),
                                          additional_forward_args=(self.preprocessor.attention_mask,0))
        attributions_end = lig.attribute(inputs=(self.preprocessor.input_ids),
                                          baselines=(self.preprocessor.baseline_input_ids),
                                          additional_forward_args=(self.preprocessor.attention_mask, 1))
        ...
        top_words_start, top_words_val_start, top_word_ind_start = get_top_k_attributed_tokens(attributions_start_word, k=k, preprocessor=self.preprocessor)
        top_words_end, top_words_val_end, top_words_ind_end = get_top_k_attributed_tokens(attributions_end_word, k=k,preprocessor=self.preprocessor)
...
class AnalyzeMaskedLM():
    ...
    def predict(self, input_ids=None, attention_mask=None) -> list:
        ...
        if input_ids is None:
            ...
            #todo: middle step for predictions[0] to list for readability
